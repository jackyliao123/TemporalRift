	...
	, m_windowInitialised(false)
{
	...
	CameraManager::GetInstance()->GetCameraView().setSize(sf::Vector2f(WINDOW_WIDTH * 2, WINDOW_HEIGHT * 2));
}
...
void GameEngineMain::UpdateWindowEvents()
{
	...
	while (m_renderWindow->pollEvent(event))
	{
		if (event.type == sf::Event::Closed)
		{
			...
			CameraManager::GetInstance()->GetCameraView().setSize(event.size.width * 2, event.size.height * 2);
		}
	}
}
